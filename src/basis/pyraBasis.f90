...
! Private Part ---------------------------------------------------------------------------------------------------------------------
! Public Part ----------------------------------------------------------------------------------------------------------------------
INTERFACE getPyraBasis
  MODULE PROCEDURE getPyraBasis
END INTERFACE

INTERFACE getBasisMappingPyra
  MODULE PROCEDURE getBasisMappingPyra
END INTERFACE

PUBLIC::getPyraBasis,getBasisMappingPyra
!===================================================================================================================================
...
INTEGER                 :: i,j,iNode  ! ?
!===================================================================================================================================
CALL getBasisMappingPyra(Deg,nAns,bMap)
CALL getBasisMappingPyra(nNodes1D-1,nNodes,nodeMap)
...
END DO
CALL VandermondePyra(nNodes,nAns,bMap,r,s,t,VdM)
WRITE(*,*)'Vandermonde'
DO j=1,nAns
...
DO iNode=1,nNodes
  ...
  WRITE(*,*)' '
END DO
CALL GradVandermondePyra(nNodes,nAns,bMap,r,s,t,gradVdM)
...
IF(PRESENT(bMapInv))THEN
  ...
  END DO
END IF
END SUBROUTINE getBasisMappingPyra


SUBROUTINE rst2abcPyra(nNodes,r,s,t,a,b,c)
!===================================================================================================================================
! Transforms reference coordinates of the pyramid (r,s,t), r,s,t>=-1, r+t<=1, s+t<=1
...
DO iNode=1,nNodes
  IF(ABS(1.-t(iNode)).LT.PP_RealTolerance)THEN
    ...
    b(iNode)=-1.
  ELSE
    a(iNode) = 2.*(1.+r(iNode))/(1.-t(iNode))-1.
...
DO iAns=1,nAns
  ...
  IF(ij.GT.1) dPhi_db=dPhi_db*((1.-c)**(ij-1))
  !dphi/dc
  dPhi_dc=dPc
  IF(ij.GT.0)dPhi_dc=dPhi_dc*(1.-c)**(ij)
  IF(ij.EQ.1)dPhi_dc=dPhi_dc-Pc
  IF(ij.GT.1)dPhi_dc=dPhi_dc-(ij)*(1.-c)**(ij-1)*Pc
